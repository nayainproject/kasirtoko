...
class TransaksiController extends Controller
{
    ...
    {
        // Perbarui status transaksi menjadi 'batal'
        $transaksi->update([
            'status' => 'batal'
        ]);

        // Ambil semua detail penjualan terkait transaksi yang dibatalkan
        $detilPenjualan = DetilPenjualan::where('penjualan_id', $transaksi->id)->get();

        // Lakukan pengembalian stok untuk setiap produk dalam detail penjualan
        foreach ($detilPenjualan as $detil) {
            $produk = Produk::find($detil->produk_id);

            if ($produk) {
                // Tambahkan kembali jumlah produk yang dibeli ke stok
                $produk->increment('stok', $detil->jumlah);
            }
        }

        // Redirect kembali dengan pesan sukses atau ke halaman sebelumnya
        return redirect()->route('transaksi.index')->with('success', 'Transaksi berhasil dibatalkan. Stok produk telah dikembalikan.');
    }
